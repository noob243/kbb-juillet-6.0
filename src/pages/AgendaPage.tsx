...
const AgendaPage: FC<AgendaPageProps> = ({ tasks, cases, lawyers, avocats = [], onAddTask, onUpdateTask, events = [] }) => {
    ...
    const getStatusClass = (status: string) => {
        switch (status) {
            ...
            case 'Effectué à moitié': return 'bg-yellow-100 text-yellow-800';
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-gray-150 shadow-xs flex items-center justify-between">
                        <div className="space-y-1">
                            ...
                            <div className="flex items-baseline gap-2">
                                <span className={`text-2xl font-extrabold ${overdueTasksCount > 0 ? 'text-red-600' : 'text-slate-800'}`}>{overdueTasksCount}</span>
                                {overdueTasksCount > 0 && (
                        ...
                        <div className="p-4 flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-50 border-b border-gray-150">
                            ...
                            <div className="flex items-center space-x-2">
                                <button 
                                    type="button"
                                    onClick={handleToday}
                                    className="bg-white hover:bg-gray-100 text-gray-800 font-bold px-3 py-1.5 rounded-lg border border-gray-200 text-xs transition duration-200"
                                >
                                    Aujourd'hui
                                </button>
                                <div className="inline-flex rounded-lg border border-gray-200 bg-white shadow-2xs">
                                    <button 
                                        type="button"
                                        onClick={handlePrevMonth}
                                        className="p-1.5 hover:bg-gray-50 border-r border-gray-200 rounded-l-lg text-gray-600 hover:text-gray-900 transition"
                                    ...
                                    >
                                        ...
                                        <div className="flex justify-between items-start">
                                            ...
                                            <div className="flex gap-1 font-mono">
                                                {dayTasks.length > 0 && (
                                                    <span className="text-[8px] font-black bg-indigo-50 text-indigo-600 px-1 py-0.2 rounded border border-indigo-100" title={`${dayTasks.length} Tâche(s)`}>
                                                        T: {dayTasks.length}
                                                    </span>
                                                )}
                                                {dayEvents.length > 0 && (
                                                    <span className="text-[8px] font-black bg-purple-50 text-purple-600 px-1 py-0.2 rounded border border-purple-100" title={`${dayEvents.length} Événement(s)`}>
                                                        E: {dayEvents.length}
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            {cellItems.slice(0, 3).map(item => {
                                                if (item.type === 'event') {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <CalendarIcon className="w-2.5 h-2.5 mr-0.5 text-purple-600 shrink-0" />
                                                    ...
                                                    if (task.status === 'Effectué') {
                                                        ...
                                                        badgeColor = 'bg-yellow-50 text-yellow-700 border-yellow-150 hover:bg-yellow-100';
                                                    }
                                                    return (
                                                        <div 
                                                            key={item.id}
                                                        ...
                                                        >
                                                            <CheckSquareIcon className="w-2.5 h-2.5 mr-0.5 text-indigo-600 shrink-0" />
                                                                                ...
                                                                                <div>
                                                                                    ...
                                                                                    <div className="flex flex-wrap gap-1">
                                                                                        {task.associatedLawyers.map(lawyer => (
                                                                                            <span key={lawyer} className="px-1.5 py-0.5 bg-indigo-50 text-indigo-700 border border-indigo-150 font-bold rounded-lg text-[10px]">
                                                                                                {lawyer}
                                                                                            </span>
                                                                                        ))}
                                                                                    </div>
                                                                                ...
                                                                                <div className="flex flex-wrap gap-1.5 p-3 bg-white border border-gray-150 rounded-xl">
                                                                                    {task.attachments.map((file, idx) => (
                                                                                        <button 
                                                                                            ...
                                                                                            className="inline-flex items-center gap-1.5 bg-slate-50 hover:bg-slate-100 text-indigo-900 border border-slate-200 text-[11px] font-bold px-2 py-1 rounded-lg transition"
                                                                                        >
                            ...
                            <div className="space-y-4 text-sm">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-slate-50 border border-gray-150 p-3 rounded-xl">
                                    ...
                                    <div>
                                        ...
                                        <p className={`font-bold text-xs ${isOverdue(selectedTaskForModal) ? 'text-red-700' : 'text-gray-800'}`}>{new Date(selectedTaskForModal.endDate || selectedTaskForModal.dueDate).toLocaleDateString('fr-FR')}</p>
                                    </div>
                                </div>
