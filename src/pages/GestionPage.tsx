...
const GestionPage: FC<GestionPageProps> = (props) => {
    ...
    const deleteButtonClass = userCanDelete
        ? "font-extrabold text-rose-600 hover:text-rose-850 bg-rose-50 hover:bg-rose-100 px-2 py-1.5 rounded-lg transition cursor-pointer"
        : "font-extrabold text-slate-400 bg-slate-100 px-2 py-1.5 rounded-lg cursor-not-allowed opacity-60";
    ...
    const handleUpdateCorrespondance = async (updated: Correspondance) => {
        try {
            ...
            await dbCreateAuditLog({
                ...
                userEmail: props.currentUser?.email || 'admin@kbb.cd',
                userName: props.currentUser?.fullName || 'Administrateur',
    ...
    const startEdit = (type: 'client' | 'case' | 'avocat' | 'personnel' | 'event' | 'task' | 'invoice' | 'fournisseur' | 'correspondance' | 'procedure', item: any) => {
        setEditingType(type);
                                                ...
                                                <>
                                                    <td className={`${tableCellClass} font-bold text-gray-800`}>
                                                        <div className="flex items-center gap-2">
                                                            {item.photo ? (
                                                                <img src={item.photo} alt="" className="w-6 h-6 rounded-full object-cover border border-gray-150" referrerPolicy="no-referrer" />
                                                            ) : (
