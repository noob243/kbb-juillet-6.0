...
const DashboardPage: FC<DashboardPageProps> = ({ clients = [], cases = [], events = [], tasks = [], invoices = [], avocats = [], personnels = [], onUpdateTaskStatus, onAddTask, onNavigate }) => {
    ...
    const activeCases = safeCases.filter(c => c && (c.status === 'En cours' || c.status === 'Nouveau'));
    const upcomingEvents = safeEvents.filter(e => e && e.date && new Date(e.date) >= new Date());
    const pendingTasks = safeTasks.filter(t => t && t.status !== 'Effectué');

    // Weekly distribution calculation for new cases and client arrivals
    const weeklyData = useMemo(() => {
        ...
        const getDayIndex = (dateStr?: string, fallbackIdx = 0) => {
            if (!dateStr) return fallbackIdx % 7;
            const dt = new Date(dateStr);
            if (isNaN(dt.getTime())) return fallbackIdx % 7;
            const dayNum = dt.getDay(); // 0 = Sunday, 1 = Monday, ...
            return dayNum === 0 ? 6 : dayNum - 1;
        };

        // Populate cases count
        safeCases.forEach((c, idx) => {
            const dateVal = (c as any).createdAt || c.procedureDateDebut || (c as any).date || '';
            const dayIdx = getDayIndex(dateVal, idx);
            counts[dayIdx].nouveauxCas += 1;
        });

        // Populate clients count
        safeClients.forEach((cl, idx) => {
            const dateVal = (cl as any).createdAt || (cl as any).createdDate || (cl as any).date || '';
            const dayIdx = getDayIndex(dateVal, idx + 1);
            counts[dayIdx].nouveauxClients += 1;
        });

        return counts;
    }, [safeCases, safeClients]);
    ...
    const clientsStats = clients.map(cl => {
        const clientCases = cases.filter(c => {
            if (!c) return false;
            if (c.clientId && cl.id && c.clientId === cl.id) return true;
            if (c.client && cl.name) {
                const caseClientLower = c.client.toLowerCase().trim();
                const clNameLower = cl.name.toLowerCase().trim();
                if (caseClientLower === clNameLower) return true;
                if (caseClientLower.includes(clNameLower) || clNameLower.includes(caseClientLower)) return true;
            }
            return false;
        });
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard title="Dossiers Actifs" value={activeCases.length.toString()} icon={<CasesIcon />} onClick={() => onNavigate?.('Dossiers')} />
                <StatCard title="Clients" value={clients.length.toString()} icon={<ClientsIcon />} onClick={() => onNavigate?.('Clients')} />
                <StatCard title="Événements à Venir" value={upcomingEvents.length.toString()} icon={<EventsIcon />} onClick={() => onNavigate?.('Evenements')} />
                <StatCard 
            ...
            {/* Weekly Distribution Chart Component (Recharts) */}
            <div className="bg-white dark:bg-[#0c111d] p-6 rounded-2xl shadow-xs border border-gray-100 dark:border-slate-800 transition-all">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
                    <div>
                        <h3 className="text-base font-black text-gray-800 dark:text-slate-100 tracking-tight flex items-center gap-2">
                            <span className="p-1.5 bg-indigo-50 dark:bg-indigo-950/40 text-[#15447c] dark:text-indigo-400 rounded-lg">
                                <svg className="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                                </svg>
                            </span>
                            Répartition Hebdomadaire : Nouveaux Cas & Arrivées Clients
                        </h3>
                        <p className="text-[10px] text-gray-400 dark:text-slate-400 font-bold uppercase tracking-wider mt-0.5">
                            Graphique à barres montrant la distribution quotidienne des nouveaux cas et des arrivées de clients
                        </p>
                    </div>
                    <div className="flex items-center gap-2">
                        <span className="px-3 py-1 bg-indigo-50 dark:bg-indigo-950/40 text-[#15447c] dark:text-indigo-300 font-black text-xs rounded-xl border border-indigo-100 dark:border-indigo-900/40">
                            {safeCases.length} Cas • {safeClients.length} Clients
                        </span>
                    </div>
                </div>
                ...
                <div className="h-[280px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={weeklyData} margin={{ top: 15, right: 15, left: -20, bottom: 0 }}>
                            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                            <XAxis dataKey="shortDay" tick={{ fontSize: 11, fill: '#64748b', fontWeight: 700 }} axisLine={false} tickLine={false} />
                            ...
                            <Bar name="Nouveaux Cas / Dossiers" dataKey="nouveauxCas" fill="#15447c" radius={[6, 6, 0, 0]} barSize={22} />
                            <Bar name="Arrivées de Clients" dataKey="nouveauxClients" fill="#10b981" radius={[6, 6, 0, 0]} barSize={22} />
                        </BarChart>
                    </ResponsiveContainer>
                </div>
            </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-xs border border-gray-150 flex flex-col justify-between">
                    <div>
                        <div className="flex items-center justify-between mb-5">
                            ...
                            <button 
                                onClick={() => onNavigate?.('Avocats')}
                                className="text-2xs font-extrabold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 px-2.5 py-1 rounded-lg transition cursor-pointer"
                                title="Voir la liste des avocats"
                            >
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col h-[400px]">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <button 
                            ...
                            className="text-xs font-bold text-rose-600 bg-rose-50 hover:bg-rose-100 px-2.5 py-1 rounded-full transition cursor-pointer"
                        ...
                        {upcomingEvents.length === 0 ? (
                            ...
                            <ul className="space-y-3.5">
                                {upcomingEvents.slice(0, 5).map(h => (
                                    <li 
                                        key={h.id} 
                                        onClick={() => onNavigate?.('Evenements', h.name)}
                                        className="p-3 bg-slate-50 hover:bg-indigo-50/20 rounded-xl border border-slate-100 transition duration-150 cursor-pointer group"
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col h-[400px]">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button 
                                onClick={() => onNavigate?.('Agenda')}
                                className="text-xs font-bold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 px-2.5 py-1 rounded-full transition cursor-pointer"
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col h-[400px]">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <button 
                            ...
                            className="text-xs font-bold text-teal-600 bg-teal-50 hover:bg-teal-100 px-2.5 py-1 rounded-full transition cursor-pointer"
                            title="Voir tous les dossiers"
                        >
                            {cases.length} total →
                        </button>
                    </div>
                                ...
                                <table className="w-full text-left">
                                    ...
                                    <tbody className="divide-y divide-gray-50">
                                        {cases.slice(0, 5).map(c => (
                                            <tr 
                                                key={c.id} 
                                                onClick={() => onNavigate?.('Dossiers', c.name)}
                                                className="hover:bg-indigo-50/10 transition cursor-pointer"
