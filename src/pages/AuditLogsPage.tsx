...
const AuditLogsPage: FC<AuditLogsPageProps> = ({ logs = [], currentUser, onAddToast }) => {
    ...
    // Unique users in existing logs
    const logUsers = useMemo(() => {
        const map = new Map<string, string>();
        safeLogs.forEach(l => {
            if (l && l.userEmail) {
                map.set(l.userEmail, l.userName || l.userEmail);
            }
        });
    ...
    const filteredLogs = useMemo(() => {
        ...
        return safeLogs.filter(log => {
            ...
            const matchesSearch = !q || desc.includes(q) || uName.includes(q) || uEmail.includes(q) || mod.includes(q) || act.includes(q) || id.includes(q);
            const matchesAction = actionFilter === 'all' || log.actionType === actionFilter;
            const matchesModule = moduleFilter === 'all' || log.module === moduleFilter;
            const matchesUser = userFilter === 'all' || log.userEmail === userFilter || log.userName === userFilter;
    ...
    }, [safeLogs, searchQuery, actionFilter, moduleFilter, userFilter]);
    ...
    return (
        <PageContainer>
            ...
            {/* Filter controls bar */}
            <div className="bg-white dark:bg-[#0c111d] p-4.5 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-xs mb-6 space-y-4">
                <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
                    ...
                    <div className="relative w-full lg:max-w-md">
                        ...
                        <input
                            type="text"
                            placeholder="Rechercher par module, action, description, utilisateur..."
                            value={searchQuery}
                    ...
                    <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
                        ...
                        <select
                            ...
                            className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs font-bold px-3 py-2 rounded-xl text-gray-700 dark:text-slate-350 outline-none focus:ring-2 focus:ring-indigo-500/10 cursor-pointer"
                        >
                        ...
                        {(searchQuery || actionFilter !== 'all' || moduleFilter !== 'all' || userFilter !== 'all') && (
                            <button
                                onClick={() => {
                                    setSearchQuery('');
                                    setActionFilter('all');
                                ...
                                className="text-xs font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/30 px-3 py-2 rounded-xl border border-rose-200 dark:border-rose-900/40 transition cursor-pointer flex items-center gap-1.5"
                ...
                {/* Quick module chips & Result summary */}
                <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100 dark:border-slate-800/80">
                    <div className="flex flex-wrap items-center gap-1.5">
                        <span className="text-3xs font-extrabold uppercase text-slate-400 dark:text-slate-500 mr-1">Modules :</span>
                        <button
                            onClick={() => setModuleFilter('all')}
                            className={`px-2.5 py-1 rounded-lg text-3xs font-black uppercase transition cursor-pointer ${
                                moduleFilter === 'all'
                        ...
                        {['Dossiers', 'Clients', 'Facturation', 'Agenda', 'Utilisateurs', 'Procédures', 'Correspondances'].map(mod => (
                            <button
                                key={mod}
                                onClick={() => setModuleFilter(moduleFilter === mod ? 'all' : mod)}
                                className={`px-2.5 py-1 rounded-lg text-3xs font-bold uppercase transition cursor-pointer ${
                                    moduleFilter === mod
                                        ? 'bg-[#15447c] text-white shadow-xs'
                    ...
                    <div className="text-3xs font-black uppercase text-slate-500 dark:text-slate-400 font-mono">
                        Affichage de <span className="text-indigo-600 dark:text-indigo-400">{filteredLogs.length}</span> sur {safeLogs.length} journal(ux)
                    </div>
                </div>
            </div>
