...
export const DetailedEditModal: React.FC<DetailedEditModalProps> = ({
  ...
  const handlePhotoUpload = async (field: string, e: React.ChangeEvent<HTMLInputElement>) => {
    setUploadError('');
    const file = e.target.files?.[0];
    if (file) {
      const validation = validateImageFile(file, 10);
      if (!validation.valid) {
        setUploadError(validation.error || 'Format invalide. Seuls JPG, JPEG et PNG sont acceptés.');
        return;
      }
      try {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2 flex items-center gap-4 p-3 bg-slate-50 dark:bg-[#121b2d] rounded-2xl border border-slate-150 dark:border-[#1e293b]">
                ...
                <div>
                  ...
                  />
                  <p className="text-[10px] text-gray-400 mt-1">Formats acceptés : JPG, JPEG, PNG (optimisé automatiquement)</p>
                </div>
              </div>
              <div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl space-y-3">
                <span className="text-[11px] font-black uppercase tracking-wider text-indigo-700 block">
                  🏛️ Immatriculation & Registres Légaux
                </span>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="block text-[10px] font-extrabold uppercase tracking-wide text-gray-500 mb-1">RCCM</label>
                    <input 
                      type="text" 
                      value={formData.rccm || ''} 
                  ...
                  <div>
                    <label className="block text-[10px] font-extrabold uppercase tracking-wide text-gray-500 mb-1">ID. NAT</label>
                    <input 
                      type="text" 
                  ...
                  <div>
                    <label className="block text-[10px] font-extrabold uppercase tracking-wide text-gray-500 mb-1">N° IMPÔT</label>
                    <input 
                      type="text" 
                      value={formData.nImpot || ''} 
                      onChange={(e) => handleFieldChange('nImpot', e.target.value)}
