...
export const CabinetManagementTab: FC<CabinetManagementTabProps> = ({ onAddToast }) => {
  ...
  const [settings, setSettings] = useState<CabinetSettings>({
    ...
    guceNumber: 'GUCE/KIN/2021/0489',
    taxNumber: 'A1234567B',
    rccm: 'CD/KIN/RCCM/20-B-00123',
    nationalId: '01-83-N12345P',
  ...
  useEffect(() => {
    try {
      const cached = localStorage.getItem('kbb_cabinet_settings');
      if (cached) {
        setSettings(prev => ({ ...prev, ...JSON.parse(cached) }));
      }
    } catch (e) {
      console.warn('Erreur lecture cache cabinet:', e);
    }

    // Charger les informations persistées depuis Firestore cabinet_info
    const fetchCabinetInfo = async () => {
      try {
        const snap = await getDoc(doc(db, 'cabinet_info', 'cabinet_default'));
        if (snap.exists()) {
          const data = snap.data();
          setSettings(prev => ({
            ...prev,
            cabinetName: data.name || prev.cabinetName,
            cabinetEmail: data.email || prev.cabinetEmail,
            cabinetPhone: data.phone || prev.cabinetPhone,
            address: data.adresse || data.address || prev.address,
            mainBar: data.barreauPrincipal || prev.mainBar,
            guceNumber: data.guceNumber || prev.guceNumber,
            taxNumber: data.taxNumber || prev.taxNumber,
            rccm: data.rccm || prev.rccm,
            nationalId: data.nationalId || data.identificationNationale || prev.nationalId,
          }));
        }
      } catch (err) {
        console.warn('Impossible de charger cabinet_info depuis Firestore:', err);
      }
    };
    fetchCabinetInfo();
  }, []);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      safeSetItem('kbb_cabinet_settings', JSON.stringify(settings));
      await setDoc(doc(db, 'cabinet_info', 'cabinet_default'), {
        name: settings.cabinetName,
        email: settings.cabinetEmail,
        phone: settings.cabinetPhone,
        adresse: settings.address,
        barreauPrincipal: settings.mainBar,
        guceNumber: settings.guceNumber || '',
        taxNumber: settings.taxNumber || '',
        rccm: settings.rccm || '',
        nationalId: settings.nationalId || '',
        updatedAt: new Date().toISOString()
      }, { merge: true });
      onAddToast('success', 'Paramètres et identifiants du cabinet mis à jour avec succès.');
    } catch (err) {
      console.warn('Sauvegarde Firestore différée:', err);
      ...
      <form onSubmit={handleSave} className="bg-white dark:bg-[#0c111d] p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="Avenue de la Justice, N° 14, Kinshasa - Gombe, RDC"
              className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs font-semibold"
            />
          </div>
        </div>

        {/* Section Identifiants Légaux & Fiscaux */}
        <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
          <label className="block text-2xs font-extrabold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-3">
            Immatriculation & Identifiants Légaux du Cabinet
          </label>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              ...
              <input
                type="text"
                value={settings.guceNumber || ''}
                onChange={e => setSettings({ ...settings, guceNumber: e.target.value })}
                placeholder="Ex: GUCE/KIN/2021/0489"
                className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs font-semibold"
              />
            </div>
        ...
        <div className="flex justify-end">
          <button
            type="submit"
            disabled={isSaving}
            className="px-6 py-2.5 rounded-xl bg-[#15447c] hover:bg-indigo-700 disabled:opacity-50 text-white font-bold text-xs shadow-md transition flex items-center gap-2 cursor-pointer"
          >
            {isSaving ? (
              <>
                <svg className="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              ...
              <span>Sauvegarder les paramètres du cabinet</span>
            )}
          </button>
        </div>
      </form>
