...
const AvocatModal: FC<AvocatModalProps> = ({ isOpen, onClose, onSave, avocats = [] }) => {
    ...
    const [permissions, setPermissions] = useState<ModuleKey[]>(DEFAULT_ROLE_PERMISSIONS.Avocat);
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (e.target.files && e.target.files[0]) {
            ...
            if (!validation.valid) {
                setPhotoError(validation.error || 'Format invalide. Seuls JPG, JPEG et PNG sont acceptés.');
                return;
            }
            try {
                ...
                console.error("Erreur optimisation photo avocat:", err);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        setFormData(initialFormState);
        setBankAccounts([]);
        setPermissions(DEFAULT_ROLE_PERMISSIONS.Avocat);
        onClose();
    };
                ...
                <form onSubmit={handleSubmit}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="border border-slate-200 p-4 rounded-2xl bg-slate-50/50 flex flex-col gap-2">
                                <div className="flex items-center gap-4">
                                    <div className="h-16 w-16 bg-slate-200 rounded-2xl overflow-hidden border border-slate-200 flex items-center justify-center shrink-0">
                                        {formData.photoUrl ? (
                                            <img src={formData.photoUrl} alt="Aperçu" className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                                        ) : (
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <input 
                                                ...
                                                className="w-full text-xs text-gray-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-xl file:border file:border-slate-300 file:text-xs file:font-bold file:bg-white file:text-gray-700 hover:file:bg-slate-50 cursor-pointer" 
                                            />
                                            {formData.photoUrl && (
                                                <button
                                                    type="button"
                                                    onClick={() => setFormData(prev => ({ ...prev, photo: null, photoUrl: '' }))}
                                                    className="px-2.5 py-1.5 text-2xs font-bold text-rose-600 hover:bg-rose-50 rounded-lg transition border border-rose-200 cursor-pointer"
                                                >
                                                    Retirer
                                                </button>
                                            )}
                                        </div>
                                        <p className="text-[10px] text-gray-400 mt-1">Formats acceptés : JPG, JPEG, PNG (compressé et optimisé automatiquement)</p>
                                    </div>
                                </div>
                                {photoError && (
                        ...
                        {/* Matrix of Rubrics and Module Permissions */}
