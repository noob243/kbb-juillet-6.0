...
export const UserProfileModal: FC<UserProfileModalProps> = ({
  ...
  const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (!validation.valid) {
        setErrorMsg(validation.error || 'Fichier image non valide.');
        return;
      }
      try {
        ...
        setErrorMsg(err.message || "Erreur lors de la lecture de l'image.");
      }
    }
  };
  ...
  const handleSaveInfo = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 4. Update Firebase Auth display name safely without passing base64 photoURL (bypassing Firebase Auth size limit)
      if (auth.currentUser) {
        try {
          ...
          console.warn("Could not update Firebase auth display name:", authErr);
        }
      }
          ...
          <div className="bg-gradient-to-r from-[#15447c] to-indigo-900 p-6 text-white relative">
            ...
            <div className="flex items-center space-x-4">
              <div className="h-14 w-14 rounded-2xl bg-white/15 backdrop-blur-md border border-white/20 flex items-center justify-center font-extrabold text-xl shadow-inner text-white uppercase overflow-hidden shrink-0">
                {photoUrl ? (
                  <img src={photoUrl} alt={fullName} className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                ) : (
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-900/60 rounded-2xl border border-gray-150 dark:border-slate-800 space-y-3">
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="h-16 w-16 rounded-2xl bg-gradient-to-br from-[#15447c] to-indigo-800 text-white flex items-center justify-center font-extrabold text-lg shadow-md border-2 border-white dark:border-slate-800 overflow-hidden shrink-0">
                      {photoUrl ? (
                        <img src={photoUrl} alt="Photo de profil" className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                      ) : (
                    ...
                    <div className="flex-1 space-y-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        />
                        <label
                          htmlFor="user-camera-input"
                          className="px-3.5 py-2 text-xs font-bold text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl transition cursor-pointer border border-gray-200 dark:border-slate-700 shadow-xs inline-flex items-center gap-1.5"
                        >
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      ...
                      <p className="text-[10px] text-slate-400 font-medium">
                        Formats acceptés : JPG, JPEG, PNG. Optimisation et compression automatiques (max 10 Mo).
                      </p>
                    </div>
                  </div>
                </div>
