...
const TaskModal: FC<TaskModalProps> = ({ isOpen, onClose, onSave, cases, lawyers, avocats = [] }) => {
    ...
    // Helper to auto-lookup email for selected lawyer
    const findLawyerEmail = (lawyerName: string): string => {
        if (!lawyerName) return '';
        const match = avocats.find(a => (a.fullName || '').trim().toLowerCase() === lawyerName.trim().toLowerCase());
        if (match && match.emails && match.emails.length > 0) {
            return match.emails[0];
        }
        return '';
    };
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            const defaultEmail = findLawyerEmail(defaultLawyer);
            setFormData({
                ...initialFormState,
                lawyer: defaultLawyer,
                assigneeEmail: defaultEmail,
            });
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
        ...
        setFormData(prev => {
            ...
            if (name === 'endDate') {
                ...
                const lawyerEmail = findLawyerEmail(value);
                if (lawyerEmail) {
                    next.assigneeEmail = lawyerEmail;
                }
            }
            return next;
        });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        onSave({
            ...
            emailReminder24h: formData.emailReminder24h,
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                <FormField label="Nom de la Tâche" required>
                                    <FormInput type="text" name="name" value={formData.name} onChange={handleChange} placeholder="Ex: Assister à l'audience de référé" required />
                                </FormField>
                            </div>
                        ...
                        <div className="bg-indigo-50/40 p-4 rounded-2xl border border-indigo-100/60 space-y-4">
                            <div className="flex items-center justify-between">
                                <label className="flex items-center gap-2.5 cursor-pointer select-none">
                                    ...
                                    />
                                    <div>
                                        <span className="block text-sm font-bold text-gray-800">Activer un rappel / alarme locale</span>
                                        <span className="block text-3xs text-slate-500 font-medium leading-none">Déclencher une sonnerie et une alerte sonore à l'heure prévue</span>
                                    </div>
                                </label>
                            ...
                            {/* 24h Automatic Email Reminder via Firebase Cloud Function */}
                            <div className="pt-3 border-t border-indigo-100/60 space-y-3">
                                <div className="flex items-center justify-between">
                                    <label className="flex items-center gap-2.5 cursor-pointer select-none">
                                        <input 
                                            type="checkbox"
                                            name="emailReminder24h"
                                            checked={formData.emailReminder24h}
                                            onChange={handleChange}
                                            className="h-4 w-4 text-emerald-600 border-gray-300 rounded focus:ring-emerald-500"
                                        />
                                        <div>
                                            <span className="block text-sm font-extrabold text-gray-800 flex items-center gap-1.5">
                                                <span>📧 Rappel e-mail automatique 24h avant l'échéance</span>
                                                <span className="px-1.5 py-0.5 bg-emerald-100 text-emerald-800 text-[9px] font-black uppercase rounded-md">Firebase Cloud Function</span>
                                            </span>
                                            <span className="block text-3xs text-slate-500 font-medium">Envoyer automatiquement un e-mail de rappel au responsable 24 heures avant l'échéance</span>
                                        </div>
                                    </label>
                                    <span className={`text-[10px] uppercase font-mono px-2 py-0.5 rounded-full font-bold ${formData.emailReminder24h ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-gray-150 text-gray-400'}`}>
                                        {formData.emailReminder24h ? 'Activé (24h)' : 'Désactivé'}
                                    </span>
                                </div>

                                {formData.emailReminder24h && (
                                    <div className="bg-white/80 dark:bg-slate-900/50 p-3 rounded-xl border border-emerald-100 space-y-2">
                                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                            <label className="block text-2xs font-extrabold text-gray-700 uppercase tracking-wider">
                                                E-mail du responsable (Destinataire du rappel)
                                            </label>
                                            {formData.lawyer && (
                                                <span className="text-[10px] text-indigo-600 font-bold">
                                                    Responsable : {formData.lawyer}
                                                </span>
                                            )}
                                        </div>
                                        <input 
                                            ...
                                            onChange={handleChange}
                                            placeholder="Ex: avocat.responsable@kbb-avocats.com"
                                            className="w-full p-2.5 bg-white border border-gray-300 rounded-lg text-xs font-semibold text-gray-800 focus:ring-2 focus:ring-emerald-500 focus:outline-hidden"
                                        />
                                        <p className="text-[10px] text-slate-500 leading-tight">
                                            ⚡ <strong>Fonctionnement :</strong> La tâche sera enregistrée dans Firestore avec le paramètre <code className="bg-slate-100 text-emerald-700 px-1 py-0.5 rounded font-mono">emailReminder24h: true</code>. La Firebase Cloud Function déclenchera automatiquement le courriel 24 heures avant l'échéance (<span className="font-semibold text-gray-700">{formData.endDate || formData.dueDate}</span>).
                                        </p>
                                    </div>
                                )}
                            </div>
