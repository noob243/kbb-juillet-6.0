...
const PersonnelModal: FC<PersonnelModalProps> = ({ isOpen, onClose, onSave, personnels = [] }) => {
    ...
    const [permissions, setPermissions] = useState<ModuleKey[]>(DEFAULT_ROLE_PERMISSIONS.Personnel);
    ...
    useEffect(() => {
        if (!isOpen) {
            ...
            setPreviewUrl('');
            setPermissions(DEFAULT_ROLE_PERMISSIONS.Personnel);
            setBankAccounts([]);
        }
    }, [isOpen]);
    ...
    const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            try {
                ...
                console.error("Erreur optimisation photo personnel:", err);
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex justify-center items-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl p-6 md:p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-fadeIn">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    {/* Photo upload section with circular frame preview */}
                    <div className="flex flex-col gap-2 p-4 bg-slate-50 border border-gray-150 rounded-xl">
                        <div className="flex flex-col sm:flex-row items-center gap-5">
                            <div className="relative w-20 h-20 rounded-full border-2 border-dashed border-gray-300 bg-white flex items-center justify-center overflow-hidden shrink-0 shadow-3xs">
                                {previewUrl ? (
                                    <img src={previewUrl} alt="Aperçu photo" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                                ) : (
                                ...
                                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                                    ...
                                    {previewUrl && (
                                        <button
                                            ...
                                            className="px-2.5 py-1 text-2xs font-bold text-rose-600 hover:bg-rose-50 rounded-xl transition border border-rose-200 cursor-pointer"
                                        >
                                            Retirer
                                        </button>
                                    )}
                                </div>
                                <p className="text-[10px] text-gray-400 mt-1 font-semibold">Formats acceptés : JPG, JPEG, PNG (optimisé automatiquement, max 10 Mo)</p>
                            </div>
                        </div>
                        {photoError && (
