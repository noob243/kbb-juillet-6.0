...
export function canDeleteRecord(user: AppUser | null): boolean {
  if (!user) return false;
  if (user.isDeleted) return false;
  if (user.hasAppAccess === false) return false;
  ...
  // 3. Pour tous les autres rôles (Avocats, Personnels), le droit de suppression doit impérativement leur être octroyé expressément
