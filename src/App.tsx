...
import { db, auth, createAuthAccountIfPossible } from './firebase.ts';
...
import { syncUsersWithFirestore, updateAppUser, createNewUser, syncAvocatsAndPersonnelsToUsers } from './services/userService';
import { initializeAllFirestoreCollections } from './services/initCollectionsService';
import { AppUser, ModuleKey, PersonnelCategory } from './types/rbac';
import { ALL_MODULE_PERMISSIONS, DEFAULT_ROLE_PERMISSIONS, canDeleteRecord } from './services/rbacService';
...
const saveLocalCache = <T,>(key: string, data: T[]) => {
    try {
        if (Array.isArray(data)) {
            // Trim bulky historical logs to protect browser storage quota
            const toSave = key === 'kbb_cache_auditLogs' ? data.slice(-50) : data;
...
function App() {
    ...
    const [toasts, setToasts] = useState<{ id: string, type: 'success' | 'error' | 'info' | 'warning', text: string }[]>([]);
    ...
    // Automatically sync and provision existing Avocats and Personnels into the users collection with valid passwords
    useEffect(() => {
        if (isDbConnected && (avocats.length > 0 || personnels.length > 0)) {
            syncAvocatsAndPersonnelsToUsers(avocats, personnels, usersList).catch(err => {
                console.warn("Notice during background sync of users:", err);
            });
        }
    }, [isDbConnected, avocats.length, personnels.length, usersList.length]);
    ...
    const handleLoginSuccess = (email: string) => {
        ...
        // 1. Check user from usersList (defined during creation in management)
        const foundInUsers = usersList.find(u => (u.email || '').trim().toLowerCase() === cleanEmail);
        if (foundInUsers) {
            const userInfo = {
                name: foundInUsers.fullName,
                role: foundInUsers.functionRole || foundInUsers.role || 'Collaborateur',
                email: cleanEmail,
                photoUrl: foundInUsers.photoUrl || ''
            };
            setCurrentUserInfo(userInfo);
            const prefix = foundInUsers.userType === 'Avocat' ? 'Maître ' : '';
            triggerToast('success', `Ravi de vous revoir, ${prefix}${foundInUsers.fullName} !`);
            dbCreateAuditLog({
                userEmail: userInfo.email,
                userName: userInfo.name,
                actionType: 'Connexion',
                module: 'Authentification',
                description: `Connexion de ${prefix}${userInfo.name} (${userInfo.role})`
            });
            return;
        }
    ...
    const handleAddAvocat = async (newAvocat: Avocat, password?: string) => {
        const email = (newAvocat.emails && newAvocat.emails[0]) ? newAvocat.emails[0].trim().toLowerCase() : '';
        const cleanPassword = (password || 'Cabinet2025!').trim();

        if (!email) {
            triggerToast('error', "L'adresse e-mail principale est requise pour enregistrer un avocat.");
            return;
        }

        // 1. Create or sync AppUser in users collection with password
        try {
            const existingUser = usersList.find(u => (u.email || '').trim().toLowerCase() === email);
            if (!existingUser) {
                await createNewUser({
                    email: email,
                    fullName: newAvocat.fullName,
                    userType: 'Avocat',
                    functionRole: newAvocat.cabinetRole || newAvocat.cabinetStatus || 'Avocat Collaborateur',
                    phone: newAvocat.phone || '',
                    permissions: DEFAULT_ROLE_PERMISSIONS.Avocat,
                    linkedEntityId: newAvocat.id,
            ...
            console.warn("Could not save avocat to users collection:", uErr);
        }

        // 2. Ensure Firebase Auth account exists
        await createAuthAccountIfPossible(email, cleanPassword);
        ...
        try {
            ...
            triggerToast('success', `Profil de l'avocat ${newAvocat.fullName} créé avec mot de passe opérationnel !`);
    ...
    const handleAddPersonnel = async (newPersonnel: Personnel, password?: string) => {
        const email = newPersonnel.email ? newPersonnel.email.trim().toLowerCase() : '';
        const cleanPassword = (password || 'Cabinet2025!').trim();
        const isOfficePersonnel = newPersonnel.category === 'Office';
            ...
            try {
                ...
                if (!existingUser) {
                    await createNewUser({
                        ...
                        userType: 'Personnel',
                        personnelCategory: (newPersonnel.category as PersonnelCategory) || 'Administratif',
                        functionRole: newPersonnel.role || 'Personnel',
                        phone: newPersonnel.phone || '',
                        hasAppAccess: !isOfficePersonnel,
                        permissions: !isOfficePersonnel ? DEFAULT_ROLE_PERMISSIONS.Personnel : [],
                        linkedEntityId: newPersonnel.id,
                        password: cleanPassword,
                        tempPassword: cleanPassword
                    });
                } else {
                ...
                console.warn("Could not save personnel to users collection:", uErr);
            }
        ...
        try {
            ...
            triggerToast('success', `Agent administratif "${newPersonnel.fullName}" enregistré avec accès opérationnel !`);
    ...
    const verifyCanDelete = (): boolean => {
        // 1. Les administrateurs ont le droit de suppression par défaut
        const isAdmin = currentUserObj?.role === 'Admin' || 
                        currentUserInfo?.role === 'Admin' || 
                        currentUserInfo?.role?.toLowerCase().includes('admin') ||
                        currentUserObj?.isSuperAdmin === true;
        if (isAdmin) {
            return true;
        }

        // 2. Pour les autres profils (Avocats, Personnels), le droit de suppression doit avoir été octroyé expressément
        if (!canDeleteRecord(currentUserObj)) {
            triggerToast('error', "Accès refusé : Le droit de suppression est réservé aux administrateurs par défaut ou doit vous être expressément octroyé dans la matrice des rôles.");
            return false;
        }
        return true;
    };
    ...
    const renderPage = () => {
        ...
        switch (currentPage) {
            ...
            case 'AIAssistant': return <ProtectedGuard user={currentUserObj} currentUserInfo={currentUserInfo} moduleKey="ai"><AIAssistantPage clients={filteredClients} cases={filteredCases} tasks={tasks} invoices={invoices} currentUser={currentUserObj} currentUserInfo={currentUserInfo} /></ProtectedGuard>;
            ...
            case 'Guide':
            case 'Documentation':
                return <PresentationPage onAddToast={triggerToast} />;
